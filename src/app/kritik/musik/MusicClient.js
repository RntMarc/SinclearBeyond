...
export default function MusicClient({ initialMusic }) {
  const t = useTranslations("Reviews");
  const ts = useTranslations("Reviews.search");

  const [music] = useState(initialMusic);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 text-orange-500">
              <Star
                size={16}
                fill={music.avgRating ? "currentColor" : "none"}
              />
              <span className="text-sm font-bold">
                {music.avgRating ? Number(music.avgRating).toFixed(1) : "-"}
              </span>
            </div>
