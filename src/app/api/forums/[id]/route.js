...
import { closeFriends, feedPostVotes, feedPosts, forumMembers, forums, users } from "@/lib/db/schema";
...
export async function GET(req, { params }) {
  ...
  try {
    ...
    if (!forumData?.[0]) return NextResponse.json({ error: "Forum not found" }, { status: 404 });
    const forum = forumData[0];

    // 2. Get Membership status
    const { data: membershipData } = await safeQuery(
      db.select().from(forumMembers).where(and(eq(forumMembers.forumId, forumId), eq(forumMembers.userId, userId))).limit(1)
    );
    ...
    const { data: whoMarkedMe } = await safeQuery(
      db.select({ userId: closeFriends.userId }).from(closeFriends).where(eq(closeFriends.friendId, userId))
    );
    ...
    if (usersWhoMarkedMeIds.length > 0) {
      visibilityConditions.push(
        and(eq(feedPosts.visibility, 2), inArray(feedPosts.userId, usersWhoMarkedMeIds))
      );
    }
