...
export default function DiscoverSearch({ onSearch, initialQuery = "" }) {
  ...
  return (
    <div ref={containerRef} className="w-full max-w-3xl mx-auto space-y-2">
      <div className={`bg-card border border-border rounded-3xl shadow-lg overflow-hidden transition-all duration-300 ${isExpanded ? 'ring-2 ring-primary/20' : ''}`}>
        {/* Main Search Input */}
        ...
        {/* Expanded Controls */}
        <div className={`border-t border-border bg-muted/30 px-4 py-4 space-y-4 overflow-hidden transition-all duration-300 ${isExpanded ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0 py-0 border-none'}`}>
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              ...
              {showLocationResults && (locationResults.length > 0 || loadingLocation) && (
                  ...
                  {loadingLocation ? (
                    ...
                    <div className="max-h-[200px] overflow-y-auto">
                      {locationResults.map((loc) => (
                        ...
                        >
                          <MapPin size={14} className="mt-1 shrink-0 text-muted-foreground" />
            ...
            {mode === "around" && (
              <div className="flex items-center gap-2 bg-background border border-border rounded-xl px-3 py-2 shrink-0">
                <span className="text-xs font-bold text-muted-foreground">{t("radiusLabel")}</span>
                <input
                  type="number"
                ...
                />
                <span className="text-xs text-muted-foreground">{t("radiusUnit")}</span>
              </div>
            )}
          </div>
